...
export function CheckCreate() {
  ...
  group("Pipelines API: Create a pipeline", () => {
    ...
    check(http.request("POST", `${pipelinePublicHost}/v1alpha/pipelines`, null, constant.params), {
    ...
    // Delete the pipeline
    check(http.request("DELETE", `${pipelinePublicHost}/v1alpha/pipelines/${resOrigin.json().pipeline.id}`, null, constant.params), {
...
export function CheckList() {
  ...
  group("Pipelines API: List pipelines", () => {
    ...
    check(http.request("GET", `${pipelinePublicHost}/v1alpha/pipelines?view=VIEW_FULL`, null, constant.params), {
      [`GET /v1alpha/pipelines?view=VIEW_FULL response pipelines[0] has recipe`]: (r) => r.json().pipelines[0].recipe !== null,
      [`GET /v1alpha/pipelines?view=VIEW_FULL response pipelines[0] recipe is valid`]: (r) => helper.validateRecipe(r.json().pipelines[0].recipe, false),
    });

    check(http.request("GET", `${pipelinePublicHost}/v1alpha/pipelines?view=VIEW_BASIC`, null, constant.params), {
      [`GET /v1alpha/pipelines?view=VIEW_BASIC response pipelines[0].recipe is null`]: (r) => r.json().pipelines[0].recipe === null,
    });

    check(http.request("GET", `${pipelinePublicHost}/v1alpha/pipelines?page_size=3`, null, constant.params), {
      [`GET /v1alpha/pipelines?page_size=3 response pipelines.length == 3`]: (r) => r.json().pipelines.length == 3,
    });

    check(http.request("GET", `${pipelinePublicHost}/v1alpha/pipelines?page_size=101`, null, constant.params), {
    ...
    check(http.request("GET", `${pipelinePublicHost}/v1alpha/pipelines?filter=mode=MODE_SYNC%20AND%20recipe.sources:%22${srcConnPermalink}%22`, null, constant.params), {
      [`GET /v1alpha/pipelines?filter=mode=MODE_SYNC%20AND%20recipe.sources:%22${srcConnPermalink}%22 response 200`]: (r) => r.status == 200,
      [`GET /v1alpha/pipelines?filter=mode=MODE_SYNC%20AND%20recipe.sources:%22${srcConnPermalink}%22 response pipelines.length > 0`]: (r) => r.json().pipelines.length > 0,
    });

    check(http.request("GET", `${pipelinePublicHost}/v1alpha/pipelines?filter=mode=MODE_SYNC%20AND%20recipe.destinations:%22${dstConnPermalink}%22%20AND%20recipe.models:%22${modelPermalink}%22`, null, constant.params), {
      [`GET /v1alpha/pipelines?filter=mode=MODE_SYNC%20AND%20recipe.destinations:%22${dstConnPermalink}%22%20AND%20recipe.models:%22${modelPermalink}%22 response 200`]: (r) => r.status == 200,
      [`GET /v1alpha/pipelines?filter=mode=MODE_SYNC%20AND%20recipe.destinations:%22${dstConnPermalink}%22%20AND%20recipe.models:%22${modelPermalink}%22 response pipelines.length > 0`]: (r) => r.json().pipelines.length > 0,
    });
...
export function CheckUpdate() {
  ...
  group("Pipelines API: Update a pipeline", () => {
    ...
    check(http.request("PATCH", `${pipelinePublicHost}/v1alpha/pipelines/${reqBody.id}`,
      JSON.stringify(reqBodyUpdate), constant.params), {
...
export function CheckUpdateState() {
  ...
  group("Pipelines API: Update a pipeline state", () => {
    ...
    check(http.request("DELETE", `${pipelinePublicHost}/v1alpha/pipelines/${reqBodyAsync.id}`, null, constant.params), {
...
export function CheckRename() {
  ...
  group("Pipelines API: Rename a pipeline", () => {
    ...
    // Delete the pipeline
    check(http.request("DELETE", `${pipelinePublicHost}/v1alpha/pipelines/${reqBody.new_pipeline_id}`, null, constant.params), {
