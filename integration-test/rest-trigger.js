...
export function CheckTrigger(header) {
  ...
  group("Pipelines API: Trigger a pipeline for single image and single model", () => {
    ...
    check(http.request("POST", `${pipelinePublicHost}/v1alpha/${constant.namespace}/pipelines`, JSON.stringify(reqHTTP), header), {
      "POST /v1alpha/${constant.namespace}/pipelines response status is 201 (HTTP pipeline)": (r) => r.status === 201,
    });

    check(http.request("POST", `${pipelinePublicHost}/v1alpha/${constant.namespace}/pipelines/${reqHTTP.id}/trigger`, JSON.stringify(constant.simplePayload), header), {
      [`POST /v1alpha/${constant.namespace}/pipelines/${reqHTTP.id}/trigger (url) response status is 200`]: (r) => r.status === 200,
    });


    check(http.request("DELETE", `${pipelinePublicHost}/v1alpha/${constant.namespace}/pipelines/${reqHTTP.id}`, null, header), {
