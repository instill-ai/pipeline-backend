...
export function CheckCreate(header) {
    ...
    group(`Connector API: Create destination connectors [with random "jwt-sub" header]`, () => {
        ...
        check(http.request("POST",
            ...
            [`[with random "jwt-sub" header] POST /v1alpha/${constant.namespace}/connectors response for creating HTTP destination status is 401`]: (r) => r.status === 401,
        });
...
export function CheckList(header) {
    ...
    group(`Connector API: List destination connectors [with random "jwt-sub" header]`, () => {
        ...
        check(http.request("GET", `${pipelinePublicHost}/v1alpha/${constant.namespace}/connectors?filter=connector_type=CONNECTOR_TYPE_DATA`, null, constant.paramsHTTPWithJwt), {
            [`[with random "jwt-sub" header] GET /v1alpha/${constant.namespace}/connectors response status is 401`]: (r) => r.status === 401,
        });
    });
}
...
export function CheckGet(header) {
    ...
    group(`Connector API: Get destination connectors by ID [with random "jwt-sub" header]`, () => {
        ...
        check(http.request("GET", `${pipelinePublicHost}/v1alpha/${constant.namespace}/connectors/${resCSVDst.json().connector.id}`, null, constant.paramsHTTPWithJwt), {
            [`[with random "jwt-sub" header] GET /v1alpha/${constant.namespace}/connectors/${resCSVDst.json().connector.id} response status is 401`]: (r) => r.status === 401,
        });
...
export function CheckUpdate(header) {
    ...
    group(`Connector API: Update destination connectors [with random "jwt-sub" header]`, () => {
        ...
        check(http.request(
            "PATCH",
            `${pipelinePublicHost}/v1alpha/${constant.namespace}/connectors/${resCSVDst.json().connector.id}`,
            JSON.stringify(csvDstConnectorUpdate), constant.paramsHTTPWithJwt), {
            [`[with random "jwt-sub" header] PATCH /v1alpha/${constant.namespace}/connectors/${resCSVDst.json().connector.id} response status 401`]: (r) => r.status === 401,
        });
...
export function CheckLookUp(header) {
    ...
    group(`Connector API: Look up destination connectors by UID [with random "jwt-sub" header]`, () => {
        ...
        check(http.request("GET", `${pipelinePublicHost}/v1alpha/connectors/${resCSVDst.json().connector.uid}/lookUp`, null, constant.paramsHTTPWithJwt), {
            [`[with random "jwt-sub" header] GET /v1alpha/connectors/${resCSVDst.json().connector.uid}/lookUp response status 401`]: (r) => r.status === 401,
        });
...
export function CheckState(header) {
    ...
    group(`Connector API: Change state destination connectors [with random "jwt-sub" header]`, () => {
        ...
        check(http.request("POST", `${pipelinePublicHost}/v1alpha/${constant.namespace}/connectors/${resCSVDst.json().connector.id}/disconnect`, null, constant.paramsHTTPWithJwt), {
            [`[with random "jwt-sub" header] POST /v1alpha/${constant.namespace}/connectors/${resCSVDst.json().connector.id}/disconnect response at UNSPECIFIED state status 401`]: (r) => r.status === 401,
        });

        check(http.request("POST", `${pipelinePublicHost}/v1alpha/${constant.namespace}/connectors/${resCSVDst.json().connector.id}/connect`, null, constant.paramsHTTPWithJwt), {
            [`[with random "jwt-sub" header] POST /v1alpha/${constant.namespace}/connectors/${resCSVDst.json().connector.id}/connect response at UNSPECIFIED state status 401`]: (r) => r.status === 401,
        });
...
export function CheckExecute(header) {
    ...
    group(`Connector API: Write destination connectors [with random "jwt-sub" header]`, () => {
        ...
        check(http.request("GET", `${pipelinePublicHost}/v1alpha/${constant.namespace}/connectors/${resCSVDst.json().connector.id}/watch`, null, header), {
            [`[with random "jwt-sub" header] GET /v1alpha/${constant.namespace}/connectors/${resCSVDst.json().connector.id}/watch response connector state is STATE_CONNECTED`]: (r) => r.json().state === "STATE_CONNECTED",
        })
        ...
        check(http.request("POST", `${pipelinePublicHost}/v1alpha/${constant.namespace}/connectors/${resCSVDst.json().connector.id}/execute`,
            ...
            [`[with random "jwt-sub" header] POST /v1alpha/${constant.namespace}/connectors/${resCSVDst.json().connector.id}/execute response status 401 (classification)`]: (r) => r.status === 401,
        });
...
export function CheckTest(header) {
    ...
    group(`Connector API: Test destination connectors by ID [with random "jwt-sub" header]`, () => {
        ...
        check(http.request("POST", `${pipelinePublicHost}/v1alpha/${constant.namespace}/connectors/${resCSVDst.json().connector.id}/testConnection`, null, constant.paramsHTTPWithJwt), {
            [`[with random "jwt-sub" header] POST /v1alpha/${constant.namespace}/connectors/${resCSVDst.json().connector.id}/testConnection response status is 401`]: (r) => r.status === 401,
        });
