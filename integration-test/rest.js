...
import * as pipelineConstants from "./constants.js";
  ...
  // Create Pipelines
  {
    ...
    group("Pipelines API: Get a pipeline", () => {
      check(
        http.request(
          "GET",
          `${pipelineHost}/pipelines/${res.json("pipeline.name")}`,
          null,
          {
            headers: genHeader("application/json"),
          }
        ...
        {
          [`GET /pipelines/${res.json("pipeline.name")} response status is 200`]: (r) => r.status === 200,
          [`GET /pipelines/${res.json("pipeline.name")} response pipeline.name`]: (r) => r.json().pipeline.name === createPipelineEntity.name,
          [`GET /pipelines/${res.json("pipeline.name")} response pipeline.description`]: (r) => r.json().pipeline.description === createPipelineEntity.description,
          [`GET /pipelines/${res.json("pipeline.name")} response pipeline.id`]: (r) => r.json().pipeline.id !== undefined,
          [`GET /pipelines/${res.json("pipeline.name")} response pipeline.recipe`]: (r) => r.json().pipeline.recipe !== undefined,
        }
      );
      check(
    ...
    group("Pipelines API: Update a pipeline", () => {
      check(
        ...
        {
          [`PATCH /pipelines/${res.json("pipeline.name")} response status is 200`]: (r) => r.status === 200,
          [`PATCH /pipelines/${res.json("pipeline.name")} response pipeline.name`]: (r) => r.json().pipeline.name === createPipelineEntity.name,
          [`PATCH /pipelines/${res.json("pipeline.name")} response pipeline.description`]: (r) => r.json().pipeline.description === updatePipelineEntity.description,
          [`PATCH /pipelines/${res.json("pipeline.name")} response pipeline.id`]: (r) => r.json().pipeline.id !== undefined,
          [`PATCH /pipelines/${res.json("pipeline.name")} response pipeline.recipe`]: (r) => r.json().pipeline.recipe !== undefined,
        }
      );
      check(
        http.request(
        ...
        {
          [`PATCH /pipelines/${res.json("pipeline.name")} response status is 200`]:
            (r) => r.status === 200,
        }
      );
      check(
    ...
    group("Pipelines API: Trigger a pipeline", () => {
      ...
      check(
        http.request(
          "POST",
          `${pipelineHost}/pipelines/${res.json("pipeline.name")}/outputs`,
        ...
        {
          [`POST /pipelines/${res.json("pipeline.name")}/outputs (url) response status is 200`]: (r) => r.status === 200,
          [`POST /pipelines/${res.json("pipeline.name")}/outputs (url) response output.detection_outputs.length`]: (r) =>
            r.json().output.detection_outputs.length === 1,
          [`POST /pipelines/${res.json("pipeline.name")}/outputs (url) response output.detection_outputs[0].bounding_box_objects.length`]: (r) =>
            r.json().output.detection_outputs[0].bounding_box_objects.length === 1,
          [`POST /pipelines/${res.json("pipeline.name")}/outputs (url) response output.detection_outputs[0].bounding_box_objects[0].category`]: (r) =>
            r.json().output.detection_outputs[0].bounding_box_objects[0].category === "test",
          [`POST /pipelines/${res.json("pipeline.name")}/outputs (url) response output.detection_outputs[0].bounding_box_objects[0].score`]: (r) =>
            r.json().output.detection_outputs[0].bounding_box_objects[0].score === 1,
          [`POST /pipelines/${res.json("pipeline.name")}/outputs (url) response output.detection_outputs[0].bounding_box_objects[0].bounding_box`]: (r) =>
      ...
      check(
        ...
        {
          [`POST /pipelines/${res.json("pipeline.name")}/outputs (base64) response status is 200`]: (r) => r.status === 200,
        }
      );
      ...
      check(
        http.request(
          "POST",
          `${pipelineHost}/pipelines/${res.json("pipeline.name")}/upload/outputs`,
          fd.body(),
          {
            headers: genHeader(`multipart/form-data; boundary=${fd.boundary}`),
          }
        ...
        {
          [`POST /pipelines/${res.json("pipeline.name")}/outputs (multipart) response status is 200`]: (r) => r.status === 200,
          [`POST /pipelines/${res.json("pipeline.name")}/outputs (multipart) response output.detection_outputs.length`]: (r) => r.json().output.detection_outputs.length === 1,
          [`POST /pipelines/${res.json("pipeline.name")}/outputs (multipart) response output.detection_outputs[0].bounding_box_objects[0].score`]: (r) => r.json().output.detection_outputs[0].bounding_box_objects[0].score !== undefined,
        }
      );
    });
  }
