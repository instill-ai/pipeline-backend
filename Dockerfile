...
RUN --mount=target=. --mount=type=cache,target=/root/.cache/go-build --mount=type=cache,target=/go/pkg GOOS=$TARGETOS GOARCH=$TARGETARCH go build -tags=ocr -o /${SERVICE_NAME}-migrate ./cmd/migration
...
RUN apt update && \
    apt install -y curl python3 python3-venv poppler-utils wv unrtf tidy tesseract-ocr libtesseract-dev libreoffice ffmpeg chromium qpdf && \
...
# Copy ONNX model
COPY --chown=nobody:nogroup ./pkg/component/operator/audio/v0/model/silero_vad.onnx /${SERVICE_NAME}/pkg/component/operator/audio/v0/model/silero_vad.onnx
