...
func HandleTriggerSyncPipelineBinaryFileUpload(s service.Service, w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
	...
	eventName := "HandleTriggerSyncPipelineBinaryFileUpload"
	...
	if err != nil {
		...
		s.WriteNewDataPoint(dataPoint.AddTag("status", "errored"))
		return
	}
	...
	dataPoint = dataPoint.AddField("compute_time_duration", time.Since(startTime).Seconds())
	s.WriteNewDataPoint(dataPoint.AddTag("status", "completed"))
...
func HandleTriggerAsyncPipelineBinaryFileUpload(s service.Service, w http.ResponseWriter, req *http.Request, pathParams map[string]string) {
	...
	req = injectSpanInRequest(req)
	eventName := "HandleTriggerAsyncPipelineBinaryFileUpload"
	...
	obj, err := s.TriggerAsyncPipelineBinaryFileUpload(ctx, logUUID.String(), owner, dbPipeline, model.Task, inp)
	if err != nil {
		http.Error(w, err.Error(), getHttpStatus(err))
		logger.Error(err.Error())
		return
