...
func (w *worker) ComponentActivity(ctx context.Context, param *ComponentActivityParam) (*ComponentActivityParam, error) {
	...
	err := w.repository.UpdateComponentRun(param.SystemVariables.PipelineTriggerID, param.ID, &datamodel.ComponentRun{StartedTime: startTime})
	if err != nil {
		...
		defer func() {
			...
			err = w.repository.UpdateComponentRun(param.SystemVariables.PipelineTriggerID, param.ID, componentRun)
			if err != nil {
