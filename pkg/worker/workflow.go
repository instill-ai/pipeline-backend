	...
	for iter := range wfm.GetBatchSize() {
		...
		if useInput {
			...
			if useArrayRange {
				...
				start = renderedRangeParam.(data.Array)[0].(*data.Number).GetInteger()
				stop = renderedRangeParam.(data.Array)[1].(*data.Number).GetInteger()
				if withStep {
					step = renderedRangeParam.(data.Array)[2].(*data.Number).GetInteger()
				}
			} else {
				if _, ok := renderedRangeParam.(data.Map)[rangeStart]; ok {
					start = renderedRangeParam.(data.Map)[rangeStart].(*data.Number).GetInteger()
				} else {
				...
				if _, ok := renderedRangeParam.(data.Map)[rangeStop]; ok {
					stop = renderedRangeParam.(data.Map)[rangeStop].(*data.Number).GetInteger()
				} else {
				...
				if _, ok := renderedRangeParam.(data.Map)[rangeStep]; ok {
					withStep = true
					step = renderedRangeParam.(data.Map)[rangeStep].(*data.Number).GetInteger()
				}
