...
// GeneratePipelineDataSpec implements service.Converter
	...
	if mmGeneratePipelineDataSpec.GeneratePipelineDataSpecMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.variables != nil && !minimock.Equal(*mm_want_ptrs.variables, mm_got.variables) {
				mmGeneratePipelineDataSpec.t.Errorf("ConverterMock.GeneratePipelineDataSpec got unexpected parameter variables, want: %#v, got: %#v%s\n", *mm_want_ptrs.variables, mm_got.variables, minimock.Diff(*mm_want_ptrs.variables, mm_got.variables))
			}

			if mm_want_ptrs.outputs != nil && !minimock.Equal(*mm_want_ptrs.outputs, mm_got.outputs) {
				mmGeneratePipelineDataSpec.t.Errorf("ConverterMock.GeneratePipelineDataSpec got unexpected parameter outputs, want: %#v, got: %#v%s\n", *mm_want_ptrs.outputs, mm_got.outputs, minimock.Diff(*mm_want_ptrs.outputs, mm_got.outputs))
			}

			if mm_want_ptrs.compsOrigin != nil && !minimock.Equal(*mm_want_ptrs.compsOrigin, mm_got.compsOrigin) {
				mmGeneratePipelineDataSpec.t.Errorf("ConverterMock.GeneratePipelineDataSpec got unexpected parameter compsOrigin, want: %#v, got: %#v%s\n", *mm_want_ptrs.compsOrigin, mm_got.compsOrigin, minimock.Diff(*mm_want_ptrs.compsOrigin, mm_got.compsOrigin))
			}

		} else if mm_want != nil && !minimock.Equal(*mm_want, mm_got) {
			mmGeneratePipelineDataSpec.t.Errorf("ConverterMock.GeneratePipelineDataSpec got unexpected parameters, want: %#v, got: %#v%s\n", *mm_want, mm_got, minimock.Diff(*mm_want, mm_got))
		}
...
func (m *ConverterMock) MinimockGeneratePipelineDataSpecInspect() {
	for _, e := range m.GeneratePipelineDataSpecMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Errorf("Expected call to ConverterMock.GeneratePipelineDataSpec with params: %#v", *e.params)
		}
	}
	...
	if m.GeneratePipelineDataSpecMock.defaultExpectation != nil && afterGeneratePipelineDataSpecCounter < 1 {
		if m.GeneratePipelineDataSpecMock.defaultExpectation.params == nil {
			m.t.Error("Expected call to ConverterMock.GeneratePipelineDataSpec")
		} else {
			m.t.Errorf("Expected call to ConverterMock.GeneratePipelineDataSpec with params: %#v", *m.GeneratePipelineDataSpecMock.defaultExpectation.params)
		}
	}
	// if func was set then invocations count should be greater than zero
	...
	if !m.GeneratePipelineDataSpecMock.invocationsDone() && afterGeneratePipelineDataSpecCounter > 0 {
		m.t.Errorf("Expected %d calls to ConverterMock.GeneratePipelineDataSpec but found %d calls",
			mm_atomic.LoadUint64(&m.GeneratePipelineDataSpecMock.expectedInvocations), afterGeneratePipelineDataSpecCounter)
	}
}
