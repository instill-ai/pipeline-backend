...
func compareFrame(c *qt.C, actual, expected format.Image) {
	...
	var mse float64
	step := 2 // Sample every 2nd pixel for faster comparison (less aggressive than 3)
	sampledPixels := 0
...
func extractFramesWithFFmpeg(video format.Video) ([]format.Image, error) {
	...
	err = ffmpeg.Input(inputFile).
		Output(outputPattern, ffmpeg.KwArgs{
			"vf":      "fps=1,scale=240:180", // Moderate resolution reduction for CI compatibility
			"pix_fmt": "rgb24",
			"q:v":     "6", // Better quality for more reliable comparisons
		}).
		OverWriteOutput().
		Run()
	if err != nil {
