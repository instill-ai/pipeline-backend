...
type detectActivityInput struct {
	Audio              base64Audio `json:"audio"`
	MinSilenceDuration int         `json:"min-silence-duration"`
	SpeechPad          int         `json:"speech-pad"`
}
...
func detectActivity(input *structpb.Struct, job *base.Job, ctx context.Context) (*structpb.Struct, error) {
	inputStruct := &detectActivityInput{}
	...
	sd, err := speech.NewDetector(speech.DetectorConfig{
		...
		SampleRate:           16000,
		Threshold:            0.5,
		MinSilenceDurationMs: inputStruct.MinSilenceDuration,
		SpeechPadMs:          inputStruct.SpeechPad,
	})
	if err != nil {
		return nil, fmt.Errorf("creating voice activity detector: %w", err)
	}
	...
	if err := sd.Destroy(); err != nil {
		return nil, fmt.Errorf("destroy speech detector: %w", err)
	}

	output := detectActivityOutput{
		Segments: make([]segmentData, len(segments)),
	}
	for i, s := range segments {
		output.Segments[i] = segmentData{StartTime: s.SpeechStartAt, EndTime: s.SpeechEndAt}
	}
