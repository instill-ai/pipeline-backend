...
func Test_buildGenerateContentConfig_SystemInstructionPriority(t *testing.T) {
	c := qt.New(t)
	...
	c.Assert(cfg.SystemInstruction, qt.IsNotNil)
	c.Assert(cfg.SystemInstruction.Parts, qt.HasLen, 1)
	// Should prioritize SystemInstruction over systemMessage
...
func Test_buildGenerateContentConfig_AllFieldsIntegration(t *testing.T) {
	...
	cfg := buildGenerateContentConfig(in, "fallback system message")
	...
	// Verify SystemInstruction takes priority over systemMessage
	c.Assert(cfg.SystemInstruction, qt.IsNotNil)
	c.Assert(cfg.SystemInstruction.Parts, qt.HasLen, 1)
	c.Check(cfg.SystemInstruction.Parts[0].Text, qt.Equals, systemText)
}
