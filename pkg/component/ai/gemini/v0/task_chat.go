...
func (e *execution) textGeneration(ctx context.Context, job *base.Job) error {
	...
	if streamEnabled {
		...
		for r, err := range client.Models.GenerateContentStream(ctx, in.Model, contents, cfg) {
			...
			// incremental flush
			if err := job.Output.WriteData(ctx, taskTextGenerationOutput{Texts: texts, Usage: map[string]any{}, Candidates: []candidate{}, UsageMetadata: usageMetadata{}, PromptFeedback: promptFeedback{}}); err != nil {
				job.Error.Error(ctx, err)
				return nil
			}
			resp = r
...
func renderFinal(resp *genai.GenerateContentResponse, texts []string) taskTextGenerationOutput {
	out := taskTextGenerationOutput{
		Texts:          []string{},
...
func buildReqParts(in taskTextGenerationInput) []genai.Part {
	parts := []genai.Part{}
...
func buildGenerateContentConfig(in taskTextGenerationInput, systemMessage string) *genai.GenerateContentConfig {
	...
	// Handle SystemInstruction - prioritize SystemInstruction over systemMessage
