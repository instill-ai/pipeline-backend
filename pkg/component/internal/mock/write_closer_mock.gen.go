...
// Close implements mm_io.WriteCloser
...
func (m *WriteCloserMock) MinimockCloseInspect() {
	...
	if m.CloseMock.defaultExpectation != nil && afterCloseCounter < 1 {
		m.t.Errorf("Expected call to WriteCloserMock.Close at\n%s", m.CloseMock.defaultExpectation.returnOrigin)
	}
	...
	if !m.CloseMock.invocationsDone() && afterCloseCounter > 0 {
		m.t.Errorf("Expected %d calls to WriteCloserMock.Close at\n%s but found %d calls",
...
type WriteCloserMockWriteExpectation struct {
	...
	expectationOrigins WriteCloserMockWriteExpectationOrigins
...
type WriteCloserMockWriteExpectationOrigins struct {
	origin  string
	originP string
}
...
func (mmWrite *mWriteCloserMockWrite) When(p []byte) *WriteCloserMockWriteExpectation {
	...
	expectation := &WriteCloserMockWriteExpectation{
		...
		expectationOrigins: WriteCloserMockWriteExpectationOrigins{origin: minimock.CallerInfo(1)},
	}
...
// Write implements mm_io.WriteCloser
	...
	if mmWrite.WriteMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.p != nil && !minimock.Equal(*mm_want_ptrs.p, mm_got.p) {
				mmWrite.t.Errorf("WriteCloserMock.Write got unexpected parameter p, expected at\n%s:\nwant: %#v\n got: %#v%s\n",
			...
			mmWrite.t.Errorf("WriteCloserMock.Write got unexpected parameters, expected at\n%s:\nwant: %#v\n got: %#v%s\n",
...
func (m *WriteCloserMock) MinimockWriteInspect() {
	for _, e := range m.WriteMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Errorf("Expected call to WriteCloserMock.Write at\n%s with params: %#v", e.expectationOrigins.origin, *e.params)
		}
	}
	...
	if !m.WriteMock.invocationsDone() && afterWriteCounter > 0 {
		m.t.Errorf("Expected %d calls to WriteCloserMock.Write at\n%s but found %d calls",
