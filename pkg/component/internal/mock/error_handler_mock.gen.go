...
type ErrorHandlerMockErrorExpectation struct {
	...
	expectationOrigins ErrorHandlerMockErrorExpectationOrigins
...
type ErrorHandlerMockErrorExpectationOrigins struct {
	origin    string
	originCtx string
	originErr string
}
...
func (mmError *mErrorHandlerMockError) ExpectCtxParam1(ctx context.Context) *mErrorHandlerMockError {
	...
	mmError.defaultExpectation.expectationOrigins.originCtx = minimock.CallerInfo(1)
...
func (mmError *mErrorHandlerMockError) ExpectErrParam2(err error) *mErrorHandlerMockError {
	...
	mmError.defaultExpectation.expectationOrigins.originErr = minimock.CallerInfo(1)
...
// When sets expectation for the ErrorHandler.Error which will trigger the result defined by the following
// Then helper
func (mmError *mErrorHandlerMockError) When(ctx context.Context, err error) *ErrorHandlerMockErrorExpectation {
	if mmError.mock.funcError != nil {
		mmError.mock.t.Fatalf("ErrorHandlerMock.Error mock is already set by Set")
	}

	expectation := &ErrorHandlerMockErrorExpectation{
		mock:               mmError.mock,
		params:             &ErrorHandlerMockErrorParams{ctx, err},
		expectationOrigins: ErrorHandlerMockErrorExpectationOrigins{origin: minimock.CallerInfo(1)},
	}
	mmError.expectations = append(mmError.expectations, expectation)
	return expectation
}

// Then sets up ErrorHandler.Error return parameters for the expectation previously defined by the When method

func (e *ErrorHandlerMockErrorExpectation) Then() *ErrorHandlerMock {
	return e.mock
}
...
// Error implements mm_base.ErrorHandler
	...
	if mmError.ErrorMock.defaultExpectation != nil {
		...
		if mm_want_ptrs != nil {
			...
			if mm_want_ptrs.ctx != nil && !minimock.Equal(*mm_want_ptrs.ctx, mm_got.ctx) {
				mmError.t.Errorf("ErrorHandlerMock.Error got unexpected parameter ctx, expected at\n%s:\nwant: %#v\n got: %#v%s\n",
					mmError.ErrorMock.defaultExpectation.expectationOrigins.originCtx, *mm_want_ptrs.ctx, mm_got.ctx, minimock.Diff(*mm_want_ptrs.ctx, mm_got.ctx))
			}

			if mm_want_ptrs.err != nil && !minimock.Equal(*mm_want_ptrs.err, mm_got.err) {
				mmError.t.Errorf("ErrorHandlerMock.Error got unexpected parameter err, expected at\n%s:\nwant: %#v\n got: %#v%s\n",
					mmError.ErrorMock.defaultExpectation.expectationOrigins.originErr, *mm_want_ptrs.err, mm_got.err, minimock.Diff(*mm_want_ptrs.err, mm_got.err))
			}

		} else if mm_want != nil && !minimock.Equal(*mm_want, mm_got) {
			mmError.t.Errorf("ErrorHandlerMock.Error got unexpected parameters, expected at\n%s:\nwant: %#v\n got: %#v%s\n",
...
func (m *ErrorHandlerMock) MinimockErrorInspect() {
	for _, e := range m.ErrorMock.expectations {
		if mm_atomic.LoadUint64(&e.Counter) < 1 {
			m.t.Errorf("Expected call to ErrorHandlerMock.Error at\n%s with params: %#v", e.expectationOrigins.origin, *e.params)
		}
	}
	...
	if !m.ErrorMock.invocationsDone() && afterErrorCounter > 0 {
		m.t.Errorf("Expected %d calls to ErrorHandlerMock.Error at\n%s but found %d calls",
