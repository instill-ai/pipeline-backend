...
This task synchronizes files from third-party storage for cataloging. New files are uploaded, and updated files are overwritten based on third-party metadata. Files added through other channels, like the Artifact API or additional storage services, will not be removed. Currently, only Google Drive is supported.
...
| Output | ID | Type | Description |
| :--- | :--- | :--- | :--- |
| [Files](#sync-files-files) (optional) | `uploaded-files` | array[object] | Files metadata in catalog. The metadata here is from Instill Artifact rather than third-party storage. |
| [Failure Files](#sync-files-failure-files) (optional) | `failure-files` | array[object] | Files that failed to upload. |
| Error Messages (optional) | `error-messages` | array[string] | Error messages for files that failed to upload. |
</div>

<details>
<summary> Output Objects in Sync Files</summary>

<h4 id="sync-files-files">Files</h4>
