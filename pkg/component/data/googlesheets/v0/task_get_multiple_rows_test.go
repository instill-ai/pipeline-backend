...
func TestGetMultipleRows(t *testing.T) {
	...
	ts := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if r.URL.Path == "/v4/spreadsheets/test-id/values/sheet1!3:3" && r.Method == "GET" {
			// Return mock row 3 response
			w.Header().Set("Content-Type", "application/json")
			_, _ = w.Write([]byte(`{
	...
	testCases := []struct {
		...
		expectedOutput taskGetMultipleRowsOutput
		expectErr      bool
		expectedErrMsg string
	}{
		{
			name: "ok - get multiple rows",
			input: taskGetMultipleRowsInput{
	...
	for _, tc := range testCases {
		c.Run(tc.name, func(c *qt.C) {
			...
			var capturedOutput taskGetMultipleRowsOutput
			ow.WriteDataMock.Set(func(ctx context.Context, output any) error {
				capturedOutput = *(output.(*taskGetMultipleRowsOutput))
				return nil
			})
