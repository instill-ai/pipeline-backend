-- Migration: Unify secret schema to use hash-based canonical ID
...
CREATE OR REPLACE FUNCTION generate_secret_canonical_id(uid UUID) RETURNS VARCHAR AS $$
DECLARE
    hash_bytes BYTEA;
...
UPDATE secret
SET id = generate_secret_canonical_id(uid);

-- Step 5: Add unique constraint on (namespace_id, id) if not exists
CREATE UNIQUE INDEX IF NOT EXISTS idx_secret_namespace_id ON secret(namespace_id, id);

-- Step 6: Drop the helper function
DROP FUNCTION IF EXISTS generate_secret_canonical_id(UUID);
